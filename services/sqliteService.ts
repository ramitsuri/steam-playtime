...
export async function processDatabase(fileBuffer: ArrayBuffer): Promise<StatsData> {
  ...
  if (tableNames.includes('game_dict')) {
    try {
      ...
      if (dictResults.length && dictResults[0].values.length) {
        dictResults[0].values.forEach(row => {
  ...
  // We'll search for 'play_time' table specifically.
  let sessionTable = tableNames.find(t => t.toLowerCase() === 'play_time') || '';
  let idCol = 'game_id';
  ...
  if (!sessionTable) {
    ...
    for (const tableName of tableNames) {
      ...
      const cols = info[0].values.map(row => (row[1] as string).toLowerCase());
      
      const foundId = cols.find(c => ID_COLS.includes(c));
      const foundTime = cols.find(c => TIME_COLS.includes(c));
      const foundDur = cols.find(c => DUR_COLS.includes(c));
  ...
  // REMOVED .slice(0, 10) to ensure we map names for ALL games found in the database
  ...
  return {
    totalGames: new Set(sessions.map(s => s.game_id)).size,
    totalHours: Math.round((sessions.reduce((sum, s) => sum + s.duration, 0) / 60) * 10) / 10,
    topGames,
